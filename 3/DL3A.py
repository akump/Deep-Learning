...
X = iris["data"][:, (0, 1)] # Changed to sepal length, sepal width
y = (iris["target"] == 1).astype(np.float64).reshape(-1, 1) # Iris-versicolor
...
C=2
svm_clf = MyLinearSVC(C=C, eta0 = 10, eta_d = 1000, n_epochs=40000)
svm_clf.fit(X, y)
...
plt.subplot(121)
plt.plot(X[:, 0][yr==1], X[:, 1][yr==1], "g^", label="Iris-versicolor")
plt.plot(X[:, 0][yr==0], X[:, 1][yr==0], "bs", label="Not Iris-versicolor")
...
plot_svc_decision_boundary(svm_clf2, 4, 8)
plt.xlabel("Sepal length", fontsize=14)
...
plt.plot(X[:, 0][yr==1], X[:, 1][yr==1], "g^")
plt.plot(X[:, 0][yr==0], X[:, 1][yr==0], "bs")
plot_svc_decision_boundary(sgd_clf, 4, 8)
plt.xlabel("Sepal length", fontsize=14)
plt.ylabel("Sepal width", fontsize=14)
